...
class MyOrders extends Component {
  ...
  renderProductItemListCell = (item, index, orderItem) => {
    ...
    if (item) {
      ...
      if (
        ...
        productImage = item.product_images[0] && item.product_images[0].image;
      }
        ...
        <View style={styles.orderRow}>
          <View>
            <View style={{ flexDirection: 'row' }}>
              ...
              <Text style={[styles.orderNumber, { opacity: 1, marginLeft: 0, }]}>{orderItem.order_number}</Text>
            </View>
            <View style={{ flexDirection: 'row' }}>
              <Text style={styles.orderNumber}>Delivery Time Slot : </Text>
              <Text style={[styles.orderNumber, { opacity: 1, marginLeft: 0, }]}>{orderItem.delivery_slot}</Text>
            </View>
          </View>
          {/* {orderItem.logistics_ship_rocket_enabled ? (orderItem.ship_rocket_status === 'delivered' || orderItem.ship_rocket_status === 'returned') && !orderItem.is_review_present && !orderItem.order_cancelled && <TouchableOpacity onPress={() => this.setState({ isInvalidReview: false, selectedOrderData: orderItem, showSubmitReviewModal: true })}>
              <Text style={styles.writeReview}>Write a Review</Text>
  ...
  renderMyOrderList = () => {
    if (this.props.route.params && this.props.route.params.isFromPreMyOrders && this.props.route.params && this.props.route.params.mainOrderData) {
      return (
        <ScrollView
        //style={styles.listContainer}
        >
          {this.renderMyOrderCell(this.props.route.params.mainOrderData, 0)}
          {/* <FlatList
