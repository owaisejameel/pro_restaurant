...
import styles from './SearchListingStyle';
...
class ProductListing extends React.Component {
    ...
    setNavigationHeaderConfiguration = () => {
        ...
        this.props.navigation.setOptions({
            ...
            headerTitle: () => (<Text numberOfLines={1} style={styles.headerTitleStyle}>{this.state.screenName}</Text>),
        ...
        try {
            ...
            if (this.props.route.params && this.props.route.params.isFromExplore) {
                ...
                if (this.props.route.params.categoryData && this.props.route.params.categoryData.subcategoryId) {
                    ...
                    console.log('@@@ Sub Category ===========', this.props.route.params.categoryData.subcategoryId, subCategoryId);
                }
                let data = {
                    id: categoryID,
                    uuid: DeviceInfo.getUniqueId(),
                ...
                this.props.getProductList(data, (res) => this.getProductListSuccessCallBack(res), (error) => this.getProductListFailureCallBack(error));
            } else {
                let data = {
                    uuid: DeviceInfo.getUniqueId(),
                    pageCount: this.state.pageCount
                }
                this.props.getAllProductList(data, (res) => this.getProductListSuccessCallBack(res), (error) => this.getProductListFailureCallBack(error));
            }
        } catch (err) {
            Sentry.captureException(err);
        }
    ...
    getProductListSuccessCallBack = (res) => {
        try {
            console.log('@@@ Get Search List Success CallBack ===================', this.state.pageCount);
            console.log('@@@ Get Search List Success CallBack ===================', res.data);
            ...
            console.log('@@@ Get Product List Success CallBack ===================');
        } catch (err) {
            Sentry.captureException(err);
        }
    }
    ...
    getFilterProductListSuccessCallBack = (res) => {
        try {
            console.log('@@@ Search List ==========', this.state.pageCount)
            console.log('@@@ Product List Filter**** ==========', this.state.productList)
            this.setState({
                productList: this.state.productList.concat(res.data),
                pageLoader: false,
                lastLoadCount: this.state.productList.concat(res.data).length,
        ...
        try {
            ...
            this.setState({ screenName: '' }, () => {
                ...
                this.setState({ productList: [], pageCount: 1, }, () => {
                    ...
                    this.props.getFilterProductList(data, (res) => this.getFilterProductListSuccessCallBack(res), (error) => this.getFilterProductListFailureCallBack(error));
                })
            });
        } catch (err) {
            Sentry.captureException(err);
    ...
    onApplySortByFilter = () => {
        try {
            ...
            if (filterIndex !== -1) {
                ...
                this.setState({ productList: [], isSortByEnabled: false, pageCount: 1, }, () => {
                    if (this.props.route.params.categoryData) {
                        if (this.props.route.params.isFromCategory)
                            categoryID = this.props.route.params.categoryData.id;
                        else
    ...
    getSoryByFilterParams = (itemIndex) => {
        ...
        switch (itemIndex) {
            ...
            case 3:
                ...
                break;
            case 4:
                filterQueryParams = 'order_field=price'
                break;
            case 5:
    ...
    _renderSearchResultsFooter = () => {
        return (
            <View style={styles.activityIndicator}>
                {this.state.pageCount > 1 && <ActivityIndicator size="large" color={'#000000'} />}
            </View>
        );
    };
        ...
        return (
            <TouchableOpacity onPress={() => this.props.navigation.navigate('ProductDescription', { productData: item })} style={styles.productGridStyle}>
                ...
                <View style={styles.titleContainer}>
                    ...
                    <Text style={styles.titleNameStyle}>{item.name}</Text>
          ...
          !this.state.noProductFound ?
                ...
                <FocusAwareStatusBar barStyle="dark-content" backgroundColor="#FFF" isFocused={true} />
                    <View style={styles.sortHeader}>
                        <TouchableOpacity style={{ flex: 1, justifyContent: 'center', flexDirection: 'row', height: hp('6%') }} onPress={() => this.setState({ showSortByModal: true })}>
                            ...
                            onPress={() => {
                                if(this.props.route.params && this.props.route.params.isFromExplore) {
                                    ...
                                    { onPressFilter: (filterQueryParams) => this.onPressFilter(filterQueryParams), 
                                        ...
                                        categoryID: this.props.route.params.isFromCategory ? this.props.route.params.categoryData.id : this.props.route.params.categoryData.category_id,
                                        isFromSearchListing: true,
                                    })
                                } else {
                                    this.props.navigation.navigate('Filter', 
                                    { onPressFilter: (filterQueryParams) => this.onPressFilter(filterQueryParams), 
                ...
                {<Text style={styles.showingResults}>Showing {this.state.productList.length} Results</Text>}
