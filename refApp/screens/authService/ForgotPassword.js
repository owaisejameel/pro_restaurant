...
class ForgotPassword extends React.Component {
  ...
  onPressSendLink = () => {
    try {
      ...
      this.props.sendLink(data, (res) => this.sendLinkSuccessCallBack(res), (err) => this.sendLinkFailureCallBack(err))
    } catch (err) {
      Sentry.captureException(err);
    }
  }
  ...
  sendLinkSuccessCallBack = async (res) => {
    try {
      console.log('@@@ Send Link Success CallBack ===================', res);
      ...
      <KeyboardAwareScrollView style={{ backgroundColor: ColorConstants.paleGrey }}>
        ...
        <View style={{ backgroundColor: ColorConstants.secondaryThemeGradient }}>
          ...
          <View style={styles.innerTopContainer}>
            ...
            {!this.state.sendLink && <Text style={styles.enterYourText}>Enter your registered Email or phone Number and we’ll sent you OTP to reset your password</Text>}
            <View style={styles.fieldContainer}>
              <View style={this.state.emailError ? styles.errorSectionStyle : this.state.email ? styles.SectionStyle : styles.SectionStyle1}>
                <Image source={R.images.phoneIcon} style={styles.ImageStyleEmail} />
                <TextInput
                  style={styles.input}
